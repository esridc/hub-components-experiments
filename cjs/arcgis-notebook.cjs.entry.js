...
const getUser = require('./get-user-88858550.js');
const create = require('./create-d5167bd3.js');
const getProp = require('./get-prop-581f8032.js');
const createFilters = require('./create-filters-c9a367cb.js');
const util = require('./util-333df5f7.js');
const content = require('./content-f2cad484.js');
const search = require('./search-79a183b0.js');
...
const apiTypes = content.categories.apiTypes.map((type) => type.toLowerCase());
...
function hasApiAgg(agoAggregations = {}) {
    const typeAggs = (getProp.getProp(agoAggregations, "counts") || []).filter((agg) => agg.fieldName === "type")[0];
    if (!typeAggs)
        return { hasApi: {} };
...
function buildFilter(queryFilters, key) {
    const terms = getProp.getProp(queryFilters, `${key}.terms`) || [];
    const joinType = getProp.getProp(queryFilters, `${key}.fn`);
...
function downloadableFilter(queryFilters) {
    const download = (getProp.getProp(queryFilters, "downloadable.terms") || [])[0];
    let downloadFilter;
    let typeKeywordFilter;
    if (download === "true") {
        downloadFilter = content.categories.downloadableTypes.map((type) => {
            return `type:"${type}"`;
        });
        typeKeywordFilter = content.categories.downloadableTypeKeywords.map((type) => {
...
function hasApiFilter(queryFilters) {
    const hasApiTrue = (getProp.getProp(queryFilters, "hasApi.terms") || [])[0];
    let apiFilter;
    if (hasApiTrue) {
        apiFilter = content.categories.apiTypes
            .map((type) => {
            return `type:"${type}"`;
        })
            .join(" OR ");
...
function groupIds(queryFilters) {
    const groups = getProp.getProp(queryFilters, "groupIds.terms") || [];
...
function collectionFilter(queryFilters) {
    const categories = getProp.getProp(queryFilters, "collection.terms") || [];
    const typesArr = categories.map((c) => content.getTypes(c));
    // flatten typesArr
...
function encodeAgoQuery(queryParams = {}) {
    ...
    const explicitFilters = createFilters.createFilters(queryParams.filter);
...
async function getItems(params, token, portal, authentication) {
    ...
    if (agoParams.countFields) {
        const chunkedCountFields = util.chunkArray(agoParams.countFields.split(","), MAX_COUNTFIELDS).map((fieldArrayChunk) => fieldArrayChunk.join(","));
        const promises = chunkedCountFields.map((chunk) => {
            return search.searchItems(Object.assign(Object.assign({}, agoParams), { params: {
                    token,
                    countFields: chunk,
        ...
        for (const response of responses) {
            const counts = getProp.getProp(response, "aggregations.counts") || [];
...
class PortalContent {
  ...
  canEdit(_itemId, _username) {
    return Promise.all([get.getItemGroups(_itemId), getUser.getUser(_username)])
  ...
  get(itemId) {
    return Promise.all([get.getItem(itemId), get.getItemData(itemId)])
...
let ArcgisNotebook = class {
  ...
  copyNotebook() {
    ...
    let hub = HubService.create('hub');
    hub.create(this.notebook, UserSession.UserSession.deserialize(this.session)).then((response) => {
