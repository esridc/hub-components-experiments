...
let CalciteTimePicker = class {
    ...
    this.setTime = (key, value, emit = true) => {
      switch (key) {
        default:
          return;
        case "hour":
          this.hour = typeof value === "number" ? time.formatTimePart(value) : value;
          break;
        case "minute":
          this.minute = typeof value === "number" ? time.formatTimePart(value) : value;
          break;
        case "second":
          this.second = typeof value === "number" ? time.formatTimePart(value) : value;
          break;
        case "meridiem":
          if (number.isValidNumber(this.hour)) {
  ...
  hourChanged(newHour) {
    if (this.hourDisplayFormat === "12" && number.isValidNumber(newHour)) {
      this.meridiem = time.getMeridiem(newHour);
    }
  }
  timeChangeHandler() {
  ...
  getDisplayHour() {
    ...
    if (this.hourDisplayFormat === "12") {
      return time.getMeridiemHour(this.hour);
    }
    return this.hour;
  }
  getTime() {
  ...
  connectedCallback() {
    ...
    if (number.isValidNumber(this.hour)) {
      this.hour = time.formatTimePart(parseInt(this.hour));
    }
    if (number.isValidNumber(this.minute)) {
      this.minute = time.formatTimePart(parseInt(this.minute));
    }
    if (number.isValidNumber(this.second)) {
      this.second = time.formatTimePart(parseInt(this.second));
    }
  }
  // --------------------------------------------------------------------------
  //
