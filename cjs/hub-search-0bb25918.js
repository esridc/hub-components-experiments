...
const search$1 = require('./search-79a183b0.js');
...
function encodeParams(params = {}) {
    ...
    const flatPaths = paths.filter(path => {
        return typeof getProp.getProp(params, path.join(".")) !== "object";
    });
    const parts = [];
    ...
    flatPaths.forEach(path => {
        ...
        const right = encodeURIComponent(getProp.getProp(params, path.join(".")) || "");
...
function serialize(searchParams) {
    ...
    const filters = createFilters.createFilters(searchParams);
...
function isFilterable(field) {
    return createFilters.filterSchema[field] && createFilters.filterSchema[field].type === "filter";
}
