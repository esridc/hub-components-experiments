...
const getPortalApiUrl = require('./get-portal-api-url-a814b68c.js');
...
function getUserUrl(session) {
    return getPortalUrl.getPortalUrl(session) + "/community/users/" + encodeURIComponent(session.username);
}
...
function applyInitialSchema(model, portalUrl) {
    const curVersion = getProp.getProp(model, "item.properties.schemaVersion");
    ...
    if (!curVersion || curVersion < 1) {
        ...
        if (clone.data.configurationSettings) {
            const config = util.cloneObject(clone.data.configurationSettings);
...
function upgradeToTwoDotOne(model) {
    const currVersion = getProp.getProp(model, "item.properties.schemaVersion");
    if (currVersion < 2.1) {
        const clone = util.cloneObject(model);
        // store the schemaVersion
        clone.item.properties.schemaVersion = 2.1;
        const collaborationGroupId = getProp.getProp(model, "item.properties.groupId");
...
function upgradeToTwoDotTwo(model) {
    ...
    const steps = getProp.getProp(clone, "data.steps");
    const templateIdsFromSteps = getTemplateIdsFromSteps(steps);
    const recommendedTemplates = getProp.getProp(clone, "data.recommendedTemplates") || [];
...
function getTemplateIdsFromSteps(steps) {
    ...
    if (Array.isArray(steps)) {
        templateIds = steps.reduce((acc, step) => {
            if (getProp.getProp(step, "templateIds.length")) {
...
function migrateSchema(model, portalUrl) {
    ...
    if (getProp.getProp(model, "item.properties.schemaVersion") === CURRENT_SCHEMA_VERSION) {
        return model;
    }
    else {
...
function getInitiative(id, requestOptions) {
    ...
    return Promise.all([
        ...
        .then(model => {
        return migrateSchema(model, getPortalApiUrl.getPortalApiUrl(requestOptions));
    });
}
...
const currentlyFollowedInitiatives = (user) => {
    ...
    return [...byUserTags, ...byGroupMembership].filter(util.unique);
};
...
function followInitiative(requestOptions) {
    ...
    return cleanUrl.request(getUserUrl(requestOptions.authentication), {
        ...
        const groupId = getProp.getProp(obj, "initiative.item.properties.followersGroupId");
        if (groupId) {
            // attempt to join it
        ...
        if (!obj.hasFollowersGroup) {
            ...
            tags.push(tag);
            return cleanUrl.request(getUpdateUrl(requestOptions.authentication), {
                params: { tags },
                authentication: requestOptions.authentication
            });
        }
