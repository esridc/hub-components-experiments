...
function applyInitialSchema(model, portalUrl) {
    var curVersion = index$4.getProp(model, "item.properties.schemaVersion");
    ...
    if (!curVersion || curVersion < 1) {
        ...
        if (clone_1.data.configurationSettings) {
            var config = index$4.cloneObject(clone_1.data.configurationSettings);
...
function upgradeToTwoDotOne(model) {
    var currVersion = index$4.getProp(model, "item.properties.schemaVersion");
    if (currVersion < 2.1) {
        var clone = index$4.cloneObject(model);
        // store the schemaVersion
        clone.item.properties.schemaVersion = 2.1;
        var collaborationGroupId = index$4.getProp(model, "item.properties.groupId");
...
function upgradeToTwoDotTwo(model) {
    ...
    var steps = index$4.getProp(clone, "data.steps");
    var templateIdsFromSteps = getTemplateIdsFromSteps(steps);
    var recommendedTemplates = index$4.getProp(clone, "data.recommendedTemplates") || [];
...
function getTemplateIdsFromSteps(steps) {
    ...
    if (Array.isArray(steps)) {
        templateIds = steps.reduce(function (acc, step) {
            if (index$4.getProp(step, "templateIds.length")) {
...
function migrateSchema(model, portalUrl) {
    ...
    if (index$4.getProp(model, "item.properties.schemaVersion") === CURRENT_SCHEMA_VERSION) {
        return model;
    }
    else {
...
var getUpdateUrl = function (session) { return index$3.getUserUrl(session) + "/update"; };
...
var currentlyFollowedInitiatives = function (user) {
    ...
    return index$2.__spreadArrays(byUserTags, byGroupMembership).filter(index$4.unique);
};
...
function followInitiative(requestOptions) {
    ...
    return index$2.request(index$3.getUserUrl(requestOptions.authentication), {
        ...
        var groupId = index$4.getProp(obj, "initiative.item.properties.followersGroupId");
        if (groupId) {
            // attempt to join it
        ...
        if (!obj.hasFollowersGroup) {
            ...
            tags.push(tag);
            return index$2.request(getUpdateUrl(requestOptions.authentication), {
                params: { tags: tags },
                authentication: requestOptions.authentication
            });
        }
...
const HubFollowButton = class {
    constructor(hostRef) {
         ...
         * Follow icon
         */
        this.icon = index.h("svg", { class: "follow-icon", viewBox: "0 0 120 120", width: "100%", height: "100%" }, index.h("circle", { cx: "18.385", cy: "101.615", r: "18.385" }), index.h("path", { d: "M-1.031 61c32.533 0 59 26.468 59 59s-26.467 59-59 59-59-26.468-59-59 26.467-59 59-59m0-23c-45.288 0-82 36.713-82 82s36.712 82 82 82 82-36.713 82-82-36.712-82-82-82z" }), index.h("path", { d: "M.154 23.041c53.349 0 96.75 43.402 96.75 96.75s-43.402 96.75-96.75 96.75-96.75-43.402-96.75-96.75 43.402-96.75 96.75-96.75m0-23c-66.136 0-119.75 53.615-119.75 119.75s53.614 119.75 119.75 119.75c66.135 0 119.75-53.615 119.75-119.75S66.289.041.154.041z" }));
        /**
        ...
        this.triggerFollow = () => {
            return utils.authenticateUser(this.clientid, this.orgurl).then(session => {
    ...
    render() {
        return index.h("hub-button", { text: this.callToActionText, action: this.triggerFollow, icon: this.icon });
    }
};
