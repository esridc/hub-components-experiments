...
function updateUser(requestOptions) {
    ...
    var updateUrl = getPortalUrl.getPortalUrl(requestOptions) + "/community/users/" + encodeURIComponent(username) + "/update";
    // mixin custom params and the user information, then drop the user info
    requestOptions.params = cleanUrl.__assign(cleanUrl.__assign({}, requestOptions.user), requestOptions.params);
    delete requestOptions.user;
    // send the request
    return cleanUrl.request(updateUrl, requestOptions);
}
...
function searchAnnotations(requestOptions) {
    ...
    return search$1.queryFeatures(requestOptions).then(response => {
        const users = [];
        const data = [];
        // use .reduce()?
...
function getAnnotationServiceUrl(orgId, requestOptions) {
    return search.searchItems(Object.assign({ q: `typekeywords:hubAnnotationLayer AND orgid:${orgId}` }, requestOptions)).then((response) => {
...
async function getMember(id, authentication) {
  let user = await getUser.getUser({ username: id, authentication: authentication });
...
async function getMemberEvents(authentication) {
  ...
  let portal = await search$1.getPortal(null, { authentication: authentication });
  try {
...
async function getMemberTeams(authentication) {
  let groups = await hubSearch.searchGroups({ q: "tags:initiativeCollaborationGroup",
    params: { searchUserAccess: "groupMember", num: 100 },
    authentication
  });
