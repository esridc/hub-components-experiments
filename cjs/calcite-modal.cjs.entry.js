...
let CalciteModal = class {
  ...
  renderCloseButton() {
    return !this.disableCloseButton ? (index.h("button", { "aria-label": this.intlClose, class: CSS.close, onClick: this.close, ref: (el) => (this.closeButtonEl = el), title: this.intlClose }, index.h("calcite-icon", { icon: ICONS.close, scale: this.scale === "s" ? "s" : "l" }))) : null;
  }
  renderStyle() {
