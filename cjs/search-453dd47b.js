...
function getEventServiceItem(orgId, requestOptions) {
    return search.searchItems(Object.assign({ q: `typekeywords:hubEventsLayer AND orgid:${orgId}` }, requestOptions)).then(response => {
...
function queryFeatures(requestOptions) {
    var queryOptions = getPortalUrl.appendCustomParams(requestOptions, [
        "where",
        "objectIds",
        "relationParam",
        "time",
    ...
    return cleanUrl.request(cleanUrl.cleanUrl(requestOptions.url) + "/query", queryOptions);
}
...
function buildEventResponse(features, url, requestOptions, token) {
    ...
    return search.searchItems(searchRequestOptions).then(function (siteInfo) {
