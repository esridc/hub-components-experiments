...
export class HubLicensePicker {
    ...
    renderNoLicense() {
        return (h("div", null,
            h("h3", null, "No License Selected"),
            h("p", null, "You should select a license so people know how they can re-use your content.")));
    }
    renderLicense(licenseId) {
        ...
        return (h("div", { class: "license-option" },
            h("calcite-card", { selectable: true },
                h("h3", { slot: "title" }, license.name),
                h("span", { slot: "subtitle" },
                    h("img", { slot: "", src: license.thumbnail }),
                    " ",
                    h("br", null),
                    license.summary),
                h("calcite-link", { theme: "dark", slot: "footer-leading", href: license.url }, "More info"))));
    }
    render() {
        return (h(Host, null,
            h("slot", null),
            this.license ? this.renderLicense(this.license) : this.renderNoLicense(),
            h("calcite-modal", { ref: (el) => this.pickerModal = el, "aria-labelledby": "modal-title", scale: "m", width: "m" },
                h("h3", { slot: "header", id: "modal-title" }, "Choose a License"),
                h("div", { slot: "content" },
                    h("calcite-tabs", null,
                        h("calcite-tab-nav", { slot: "tab-nav" },
                            h("calcite-tab-title", { active: true }, "Recommended Licenses"),
                            h("calcite-tab-title", null, "Custom license")),
                        h("calcite-tab", { active: true }, Object.keys(this.licenses).map(license => this.renderLicense(license))),
                        h("calcite-tab", null,
                            h("metadata-form", { sections: ['license'] })))),
                h("calcite-button", { slot: "secondary", width: "full", appearance: "outline" }, "Cancel"),
                h("calcite-button", { slot: "primary", width: "full" }, "Select License")),
            h("calcite-button", { onClick: (_ev) => this.showModal() }, "Choose a License")));
    }
