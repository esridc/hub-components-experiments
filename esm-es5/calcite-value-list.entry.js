...
var CalciteValueList = /** @class */ (function () {
    function class_1(hostRef) {
        ...
        this.keyDownHandler = function (event) {
            var handleElement = event
                .composedPath()
                .find(function (item) { var _a; return ((_a = item.dataset) === null || _a === void 0 ? void 0 : _a.jsHandle) !== undefined; });
            var item = event
                .composedPath()
            ...
            var _b = _this, el = _b.el, items = _b.items;
            var moveOffset = event.key === "ArrowDown" ? 1 : -1;
            var currentIndex = items.indexOf(item);
            var nextIndex = getRoundRobinIndex(currentIndex + moveOffset, items.length);
            if (nextIndex === items.length - 1) {
                el.appendChild(item);
            }
            else {
                var itemAtNextIndex = el.children[nextIndex];
                var insertionReferenceItem = itemAtNextIndex === item.nextElementSibling
                    ? itemAtNextIndex.nextElementSibling
                    : itemAtNextIndex;
                el.insertBefore(item, insertionReferenceItem);
            }
            requestAnimationFrame(function () { return handleElement.focus(); });
    ...
    class_1.prototype.calciteListItemRemoveHandler = function (event) {
