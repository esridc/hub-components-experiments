...
var hubLicensePickerCss = ":host{display:block}.license-option{width:100%;max-height:200px}.license-option img{height:40px}";
var HubLicensePicker = /** @class */ (function () {
    ...
    HubLicensePicker.prototype.showModal = function () {
        this.pickerModal.active = true;
    };
    HubLicensePicker.prototype.renderNoLicense = function () {
    ...
    HubLicensePicker.prototype.renderLicense = function (licenseId) {
        var license = this.licenses[licenseId];
        console.log("renderLicense", license);
        if (!license) {
            return (this.renderNoLicense());
        }
        return (h("div", { class: "license-option" }, h("calcite-card", { selectable: true }, h("h3", { slot: "title" }, license.name), h("span", { slot: "subtitle" }, h("img", { slot: "", src: license.thumbnail }), " ", h("br", null), license.summary), h("calcite-link", { theme: "dark", slot: "footer-leading", href: license.url }, "More info"))));
    };
    HubLicensePicker.prototype.render = function () {
        var _this = this;
        return (h(Host, null, h("slot", null), this.license ? this.renderLicense(this.license) : this.renderNoLicense(), h("calcite-modal", { ref: function (el) { return _this.pickerModal = el; }, "aria-labelledby": "modal-title", scale: "m", width: "m" }, h("h3", { slot: "header", id: "modal-title" }, "Choose a License"), h("div", { slot: "content" }, h("calcite-tabs", null, h("calcite-tab-nav", { slot: "tab-nav" }, h("calcite-tab-title", { active: true }, "Recommended Licenses"), h("calcite-tab-title", null, "Custom license")), h("calcite-tab", { active: true }, Object.keys(this.licenses).map(function (license) { return _this.renderLicense(license); })), h("calcite-tab", null, h("metadata-form", { sections: ['license'] })))), h("calcite-button", { slot: "secondary", width: "full", appearance: "outline" }, "Cancel"), h("calcite-button", { slot: "primary", width: "full" }, "Select License")), h("calcite-button", { onClick: function (_ev) { return _this.showModal(); } }, "Choose a License")));
    };
